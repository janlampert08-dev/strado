...
- **Migrations are applied by hand.** Green CI means nothing about the live
  ...
  **thirteen** — the eight comes from the frozen snapshot in
  ...
  - **`0096`–`0099` went in on 2026-09-15, in that order, ahead of their
    ...
    intended order: schema first). `0095` is **free**: PR #248 carries no
    migration, contrary to what #249's description assumed; the gap is
    harmless. Two things this set is worth remembering for:
