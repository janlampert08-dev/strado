...
| [`features.md`](features.md) | Die vier neuen Funktionen, die Grenze zwischen gratis und bezahlt, und was bewusst nicht gebaut wurde |
...
- **Funktionen:** Wetterfenster, Pass-Alarm, Pass-Sammlung mit
  Saisonrückblick, Wartungsheft mit MFK-Erinnerung.
...
**25 Strecken, 14 Konten, 3 Abos** (Produktionsdatenbank, 17.09.2026). Vier
...
- `docs/premium-naechste-features.md` — die Auswahl, aus der drei dieser vier
  Funktionen stammen. Ihr Massstab ("was im Winter Wert hat") ist der Grund,
  warum es diese vier sind.
- `docs/premium-ausbau-plan.md` — die UI-Regeln, an die sich jede der vier
  hält (ein Ort je Funktion, `BottomNav` unberührt, kein neues Muster). Seine
  vier geplanten Features (Abzeichen, GPX-Import, Garage, Sammlungen) sind
  damit teilweise erledigt: die Garage ist das Wartungsheft, der Rest bleibt
  offen.
