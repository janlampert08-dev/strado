...
| `dauer_sekunden` is a client-supplied clock | **Closed in code; live once `0096` is applied.** The product decision it was waiting for was taken on 2026-09-15: the leaderboard ranks only rides the server itself clocked, and everything else keeps its trail-derived duration for the rider's own statistics. `0096_fahrtstart_serverseitig.sql` adds `fahrt_starts` — a row written when the timer actually starts (`useRideRecorder` → `beginActualTracking`, so at the start point, not at the tap) and redeemed on save, with the elapsed time measured between two server clocks. `route_leaderboard` now filters `dauer_quelle = 'server'`, and the trigger `enforce_route_completion_dauer` sets that column and copies the duration off the ticket, so neither the source nor the number can be claimed by a client. The guest flow changes exactly as this row predicted: the ticket carries a secret instead of a session, rides in the tracking snapshot, and survives sign-up. **Until the migration is applied, nothing of this is live** — the app keeps writing the trail-derived duration and the column does not exist. What stays open, deliberately: the server clock keeps running while the screen sleeps, so a stop costs leaderboard time (see the PR), and per-lap rows from `lapDetection` stay trail-derived and therefore out of the leaderboard. The original wording follows, because it is the reason the shape is what it is. **Open, and it needs a product decision, not a migration.** The band from `0059` (≤ 200 km/h average) leaves room: a genuine 10 km / 600 s trail replayed with timestamps ×0.4 becomes 240 s at 150 km/h and passes. Closing it needs a ride start the server recorded itself — and the recorder is open to signed-out visitors (`GefahrenSection` takes `userId: string \| null`), so there is no session to hang that start on for a guest ride. Any fix therefore changes the guest flow. A tighter speed heuristic was considered and rejected here: every threshold that catches ×0.4 compression on a pass road also rejects real rides, and a rule that quietly discards genuine rides is worse than the forgery it prevents. |
