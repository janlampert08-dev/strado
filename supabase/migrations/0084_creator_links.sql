...
comment on table public.creator_links is
  'Vergebene Einstiegscodes fuer /c/<code>. Verwaltet unter /moderation/creator. Aufgeloest wird oeffentlich ueber creator_link_aufloesen(text), nicht ueber diese Tabelle — name ist personenbezogen und wird deshalb nicht an anon gegeben (0084).';
...
comment on policy "Moderatoren verwalten Creator-Links" on public.creator_links is
  'Die Liste gehoert der Moderation. Ein normaler eingeloggter Nutzer sieht keine einzige Zeile — das ist die Schranke, die name schuetzt, denn Spalten-Grants koennen Moderatoren nicht von uebrigen authenticated-Nutzern unterscheiden (0084).';
...
comment on function public.creator_link_aufloesen(text) is
  'Loest einen Einstiegscode fuer /c/<code> auf. SECURITY DEFINER, weil die noetige Trennung zwischen Spalten verlaeuft (name ist personenbezogen) und Spalten-Rechte pro Rolle vergeben werden — Moderator und Normalnutzer sind beide `authenticated`. Gibt nur code/kanal/kampagne eines AKTIVEN Codes zurueck und kann nicht auflisten (0084).';
