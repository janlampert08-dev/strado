...
## Eingespielt: 0096 + 0097 (Fahrtstart serverseitig, 2026-09-15, Produktion)

| Datei | Was |
| --- | --- |
| `0096_fahrtstart_serverseitig` | Tabelle `fahrt_starts`, `fahrt_start_anlegen()`, `fahrt_start_einloesen()`, Trigger `enforce_route_completion_dauer`, Spalten `dauer_quelle`/`dauer_trail_sekunden`/`fahrt_start_id`, View `route_leaderboard` auf `dauer_quelle = 'server'` verengt |
| `0097_fahrtstart_einloesen_nur_angemeldet` | **Nachtrag**, siehe unten: `revoke execute ... from anon` und `auth.uid()`-Pflicht in `fahrt_start_einloesen()` |

`0096` stammt aus PR #249 und ist **vor** dessen Merge eingespielt worden —
die vom PR selbst geforderte Reihenfolge ("Nicht mergen, bevor die Migration
steht"). `0097` liegt auf dem Branch `claude/konkurrenzanalyse-schweiz-46t0ka`
und gehört fachlich zu #249; wer #249 merged, braucht beide.

**Folge, die sofort eintrat:** die drei Zeilen, die `route_leaderboard` vorher
führte (auf einer Strecke), sind aus der Liste verschwunden, weil sie
`dauer_quelle = 'trail'` tragen. Die PR-Beschreibung sagte, die Liste sei
ohnehin leer — das stimmte nicht, gezählt wurde vorher `3`. Die Fahrten selbst
sind unberührt. Bis der Code aus #249 deployt ist, bleibt die Liste leer, weil
kein Aufrufer ein Ticket mitschickt.

### Der Nachtrag 0097 — und die Falle, die zum vierten Mal zuschlug

`0096` warnt im eigenen Kommentar vor der Falle aus `0047`, `0048` und `0091`
— Supabase vergibt jeder neuen Funktion in `public` eine **direkte**
Ausführungsberechtigung an `anon`, die ein `revoke ... from public` nicht
anfasst — und tappt dann hinein: es schreibt `revoke ... from public` plus
`grant ... to authenticated` und hält das für "positiv formuliert". Ein Grant
an `authenticated` entfernt den Grant an `anon` nicht.

Am Katalog gemessen, direkt nach dem Einspielen von `0096`:
...
where n.nspname = 'public'
  and p.proname in ('fahrt_start_anlegen', 'fahrt_start_einloesen');
-- fahrt_start_einloesen | anon = true   <-- so nicht gemeint
```

**Diesmal mit Wirkung, anders als bei `0091`.** Ein Funktionstest gegen die
Produktionsdatenbank (Zeile danach gelöscht) hat ein frisches Ticket ohne
Sitzung eingelöst: Rückgabe `12`, `verbraucht_am` gesetzt, `eingeloest_von`
**NULL**. Da der Trigger `eingeloest_von = new.user_id` verlangt, wäre dieses
Ticket danach wertlos gewesen — wer ID und Geheimnis kennt, konnte eine fremde
Fahrt um ihre Ranglistenwertung bringen. Die Fahrt selbst wäre geblieben.

Nach `0097` derselbe Test: Rückgabe `NULL`, Zeile unberührt
(`verbraucht_am`, `dauer_sekunden`, `eingeloest_von` alle NULL).

### Nach der Einspielung geprüft

```sql
-- Grants: anon darf anlegen, aber nicht einlösen
...
-- Indizes: fahrt_starts_gast_eimer_idx, fahrt_starts_offen_idx,
--          route_completions_fahrt_start_idx
-- Spalten: dauer_quelle, dauer_trail_sekunden, fahrt_start_id
```

Anmerkung zur Triggerreihenfolge: `0096` begründet sie damit, dass
`"enforce_route_completion_dauer"` alphabetisch vor
`"enforce_route_completion_stats"` komme. Der Statistik-Trigger heisst in der
Datenbank aber `route_completions_enforce_stats`. Das Ergebnis stimmt
trotzdem (`e` < `r`), die Begründung nicht.

### Was damit NICHT gemessen ist

`fahrt_start_einloesen` friert die Dauer beim **ersten** Aufruf ein, und nichts
bindet diesen Aufruf an das Ende der Fahrt. Gemessen: ein Ticket, 12 Sekunden
nach dem Anlegen eingelöst, trägt `dauer_sekunden = 12`. Wer sein eigenes
Geheimnis hält — es liegt im Tracking-Snapshot des Browsers — kann die Uhr
also mitten in der Fahrt anhalten und danach weiterfahren. Für eine 10-km-
Strecke passiert jede eingefrorene Dauer ab 180 s das 200-km/h-Band aus
`0059`. Damit ist die ×0.4-Fälschung aus A1 Bein 2 weiterhin möglich, ohne
einen einzigen Zeitstempel anzufassen.

`0097` schliesst das **nicht** — es schliesst nur den Grant und die
Entwertung durch Dritte. Der Rückweg für beide Migrationen:
...
-- View zurück auf den Stand vor 0096: dieselbe Definition ohne die Zeile
