...
export default async function PremiumAbschlussPage({
  ...
  // confirmSubscription prüft den Zustand bei Stripe nach und stellt
  // insbesondere sicher, dass das Abo dem eigenen Customer gehört — die
  // Abo-ID kommt aus der Adresszeile und ist damit eine Nutzereingabe.
  // Ohne diese Prüfung liesse sich mit einer fremden Abo-ID Premium für das
  // eigene Konto einschalten (siehe lib/actions/billing.ts).
  const bestaetigt = abo ? await confirmSubscription(abo) : false;
    ...
    <div className="flex h-dvh flex-col">
      ...
      <main className="mx-auto flex w-full max-w-md flex-1 flex-col justify-center gap-4 px-5 py-8 sm:px-6">
        ...
        <div className="flex flex-wrap gap-2">
          <Link
            href={abo ? `/profil/premium/abschluss?abo=${encodeURIComponent(abo)}` : "/profil"}
